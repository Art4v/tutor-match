...
export async function middleware(request) {
  ...
  if (user) {
    ...
    // The partner invite claim page (0063). A user claiming an invite has
    // role = null at that exact moment, so without this exemption the NULL-role
    // gate below bounces them to /choose-role — which does not offer Partner,
    ...
    const onPartnerClaim = pathname.startsWith("/partners/claim");
    const exempt =
      onChooser ||
      onDisabled ||
      onPartnerClaim ||
