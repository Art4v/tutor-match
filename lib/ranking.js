...
// their completion score (see rankingScore): all verified tutors therefore share
// the same ranking score and are ordered purely by the tie-breaker, i.e. ranked
// EQUALLY among themselves. The value is deliberately larger than the maximum
// possible completion score (the sum of all criteria weights is 16 today), so
// ANY verified tutor outranks EVERY unverified one regardless of completeness.
...
/**
 * Weighted ranking score for ordering. Verified tutors all get the same flat
 * VERIFIED_SCORE — so they rank equally among themselves and always above every
 * unverified tutor. Unverified tutors are scored by profile completeness, which
 * equals `completionScore(t).done` while every weight is 1 and diverges once
 * weights are tuned in RANKING_CONFIG.
 */
export function rankingScore(t) {
  if (!t) return 0;
  if (t.verified) return VERIFIED_SCORE;
