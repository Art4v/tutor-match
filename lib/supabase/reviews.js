...
/**
 * Every APPROVED review for one CENTRE, newest first (0067). Reviews of a
 * partner attach to the centre, never to the individual tutors it lists, so
 * this is the only review read a partner surface makes.
 */
export async function getPartnerReviews(supabase, partnerId) {
  if (!partnerId) return [];
  const { data, error } = await supabase.rpc("get_partner_reviews", { p_partner_id: partnerId });
  if (error) return [];
  return (data ?? []).map(reviewRowToCard);
}
...
export async function getReviewForModeration(admin, reviewId) {
  ...
  const { data: review } = await admin
    .from("reviews")
    .select("id, tutor_id, partner_id, student_id, rating, body, status, created_at")
    .eq("id", reviewId)
    .maybeSingle();
  ...
  const { data: author } = await admin
    ...
    .maybeSingle();
  const studentName = author?.full_name || "A student";

  // A review points at EITHER a tutor or a centre (0067, CHECK-enforced), so
  // exactly one of these branches runs. `subjectName` / `subjectHref` are what
  // the moderation surfaces render, which is why they don't care which it was.
  if (review.partner_id) {
    ...
    return {
      ...
      subjectHref: partner?.slug ? `/partners/${partner.slug}` : null,
      // Who gets told the review was published. NULL for an unclaimed centre,
      // which is a real state (we create the page before anyone owns it), so
      // every caller must treat notifying as optional rather than assured.
      notifyUserId: partner?.owner_id ?? null,
      studentName,
    };
  }
  ...
  return {
    ...
    subjectHref: tutor?.slug ? `/tutor/${tutor.slug}` : null,
    notifyUserId: review.tutor_id,
    studentName,
    // Back-compat aliases: /admin/review and the approve/reject routes were
    // written against these before centres existed.
 ...
 */
export async function getMyReviewForPartner(supabase, studentId, partnerId) {
  if (!studentId || !partnerId) return null;
