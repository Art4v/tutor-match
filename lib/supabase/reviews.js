...
/**
 * Every APPROVED review for one COMPANY, newest first (0067). Reviews of a
 * company attach to the company, never to the individual tutors it lists, so
 * this is the only review read a company surface makes.
 */
export async function getCompanyReviews(supabase, companyId) {
  if (!companyId) return [];
  const { data, error } = await supabase.rpc("get_partner_reviews", { p_partner_id: companyId });
  if (error) return [];
  return (data ?? []).map(reviewRowToCard);
}
...
export async function getReviewForModeration(admin, reviewId) {
  ...
  // A review points at EITHER a tutor or a company (0067, CHECK-enforced), so
  ...
  if (review.partner_id) {
    ...
    return {
      ...
      subjectHref: company?.slug ? `/companies/${company.slug}` : null,
      // Who gets told the review was published. NULL for an unclaimed company,
      ...
      notifyUserId: company?.owner_id ?? null,
      studentName,
    };
  }
  ...
  return {
    ...
    // written against these before companies existed.
 ...
 */
export async function getMyReviewForCompany(supabase, studentId, companyId) {
