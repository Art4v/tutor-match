...
export async function listPartners(supabase, { q = "", states = [], subjectSlugs = [] } = {}) {
  ...
  let query = supabase
    .from("partners")
    .select(`${PARTNER_SELECT}, tutors:tutor_profiles ( id, visibility )`)
  ...
  return data
    .map((row) => ({
      ...
      tutorCount: (row.tutors ?? []).filter((t) => t.visibility === "public").length,
    }))
...
const PARTNER_TUTOR_SELECT = `
  id, slug, bio, avatar_url, avatar_bg, initials, visibility,
