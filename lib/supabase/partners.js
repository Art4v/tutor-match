 ...
 * new SQL because 0065's "tutor_profiles partner write" policy is `for all`
 * over the centre's own tutors with no column restriction, the same way
...
export async function savePartnerTutor(supabase, tutorId, tutor) {
  ...
  const payload = {
    ...
    profile: {
      ...
      // the stored value, so leaving them out was safe but meant a centre could
  ...
  // and partner tutors are ordinary rows in that feed (0065).
