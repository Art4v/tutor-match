...
export async function saveTutorProfile(supabase, id, tutor) {
  try {
    // Full name is required. Guard here (not just in the editor UI) so any caller
    // of this helper is held to the same rule; the DB also enforces it with a
    // CHECK constraint on profiles.full_name (migration 0016) as the backstop.
