...
// display. Exported alongside extractAtarFromCredentials because the PARTNER
// tutor save path (lib/supabase/partners.js) writes the same column through a
...
function tutorRowToCard(row) {
  ...
  return {
    ...
    // A centre's tutor (0065). The card renders this chip in place of the
    // verified rosette, and `verified` below is forced true so the default
    // verified-only browse filter doesn't hide them: only invited centres exist,
    // so every partner tutor on the site belongs to one we chose to list.
    partner: row.partner ? { slug: row.partner.slug, name: row.partner.name } : null,
...
function rowToCompletion(row, name) {
  return {
    ...
    // Partner tutors ride along for the same reason they pass the verified
    // filter: an invited centre is the endorsement. Within the verified block
...
export async function getTutorsForBrowse(supabase, params = {}) {
  ...
  // A centre's tutors count as verified (0065): partners are invite-only, so
  // every one of them belongs to a centre we chose to list. This has to be in
  // the SQL, not just tutorRowToCard — the filter runs first, and without the
  // OR the default verified-only browse would hide every partner tutor and
  ...
  const { data, error } = await supabase
    ...
    .select(
      `
        *,
        profile:profiles!inner ( full_name ),
        partner:partners ( id, slug, name, website_url, logo_url ),
        subjects:tutor_subjects (
          position,
          subject:subjects ( id, name, slug, exam_code )
        ),
...
function tutorRowToDetail(row) {
  ...
  return {
    ...
    // The centre this tutor belongs to, or null for an ordinary independent
    // tutor (0065). Non-null changes three things on the profile page: the
    // partner chip replaces the verified tick, the centre's rate card replaces
    ...
    partner: row.partner
      ? {
          ...
          logoImg: row.partner.logo_url ?? null,
        }
      : null,
