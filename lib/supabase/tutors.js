...
/**
 * Like `getSubjectNames` but also returns slugs — used by /browse and the home
 * page so the UI can label by name while putting slugs in URLs.
 */
...
const BROWSE_SELECT = `
  ...
  subjects:tutor_subjects ( subject:subjects ( name, slug ) )
`;
 ...
 *
 * params: {
 *   q?:            text search across name / headline / city / suburb
...
export async function getTutorsForBrowse(supabase, params = {}) {
  ...
  // If filtering by subject, first resolve which tutor_ids have any of them.
  // Two round-trips is fine at our scale and keeps the main query simple.
  let subjectFilteredIds = null;
  if (subjectSlugs.length > 0) {
    ...
    subjectFilteredIds = Array.from(new Set((links ?? []).map((r) => r.tutor_id)));
  ...
  if (q) {
    const like = `%${q}%`;
    // headline/city/suburb live on tutor_profiles; full_name lives on the
    // joined profiles row. PostgREST supports the latter via dotted syntax.
    query = query.or(
      `headline.ilike.${like},city.ilike.${like},suburb.ilike.${like},profile.full_name.ilike.${like}`,
    );
  }
  ...
  if (mode === "online") query = query.eq("online", true);
  else if (mode === "inperson") query = query.eq("delivers_in_person", true);
...
/**
 * Min/max ATAR and rate across all public tutor profiles. Powers the dynamic
 * range of the ATAR / rate sliders in /browse so they always span exactly the
 * values that are actually listed. Returns sensible fallbacks when there's no
 * data (so the sliders still render with a usable range).
 */
export async function getFilterBounds(supabase) {
  const fallback = { atarMin: 90, atarMax: 99.95, rateMin: 30, rateMax: 200 };
  ...
  const atars = data.map((r) => Number(r.atar)).filter((n) => Number.isFinite(n) && n > 0);
  const rates = data.map((r) => Number(r.rate)).filter((n) => Number.isFinite(n) && n > 0);

  return {
    atarMin: atars.length ? Math.min(...atars) : fallback.atarMin,
    atarMax: atars.length ? Math.max(...atars) : fallback.atarMax,
    rateMin: rates.length ? Math.min(...rates) : fallback.rateMin,
    rateMax: rates.length ? Math.max(...rates) : fallback.rateMax,
  };
}

/**
 * Distinct cities currently represented by public tutor profiles. Powers the
 * city <select> in /browse. Returns an array of non-empty strings, sorted.
 */
export async function getDistinctCities(supabase) {
  const { data, error } = await supabase
    .from("tutor_profiles")
    .select("city")
    .eq("visibility", "public")
...
// Default labels for the availability grid when the stored value is a bare
// 2D array (which is how the dashboard editor currently writes it).
const DEFAULT_AVAILABILITY_HOURS = ["8am", "10am", "12pm", "2pm", "4pm", "6pm", "8pm", "10pm"];
const DEFAULT_AVAILABILITY_DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

function normalizeAvailability(raw) {
  if (!raw) return null;
  if (Array.isArray(raw)) {
    // 2D grid; pair with sensible default labels.
    return {
      hours: DEFAULT_AVAILABILITY_HOURS.slice(0, raw.length),
      days:  DEFAULT_AVAILABILITY_DAYS.slice(0, raw[0]?.length ?? 7),
      grid:  raw,
    };
  }
...
function tutorRowToDetail(row) {
  ...
  return {
    ...
    subjects: (row.subjects ?? []).map((s) => s?.subject?.name ?? s?.name).filter(Boolean),
