...
export async function getTutorProfileForEditor(supabase, id) {
  ...
  return {
    ...
    // the RequestVerification card. Server-controlled (saveTutorProfile never
    // writes it); flipped to 'verified' only by the approve route.
    ...
    credentials: bridgeAtarIntoCredentials(profileRow.credentials, profileRow.atar),
    bio: profileRow.bio ?? "",
...
// ATAR lives in the `atar` scalar column (so /browse can filter by atar_min in SQL),
// but is surfaced to the UI as a credential entry alongside awards/degrees so tutors
// without an ATAR aren't forced to fill a dedicated field. The split is round-tripped
// here on load and re-merged in saveTutorProfile on write.
function bridgeAtarIntoCredentials(credentials, atar) {
  const list = Array.isArray(credentials) ? credentials.filter((c) => c && c.icon !== "atar") : [];
  if (atar) list.unshift({ label: Number(atar).toFixed(2), icon: "atar" });
  return list;
}
  ...
  try {
    ...
    const { error: tErr } = await supabase
      ...
      .update({
        ...
        // NB: `verified` and `verification_status` are intentionally NOT written
        // here — they're server-controlled (only the approve route / RPC flow can
        // set them), so a tutor can't self-verify by editing their profile.
        ...
        credentials: (tutor.credentials ?? []).filter((c) => c?.label && c.icon !== "atar"),
        bio: tutor.bio || null,
        bio_long: tutor.bioLong || null,
        atar: extractAtarFromCredentials(tutor.credentials),
    ...
    // Resolve listed-school slugs → school_id FKs for the education rows. The
    // editor (SchoolCombobox) works in slugs; only high-school rows carry one.
    const { data: schoolRows } = await supabase.from("schools").select("id, slug");
    ...
    await replaceAll(supabase, "tutor_education", id,
      (tutor.education ?? [])
        ...
        .map((e, i) => ({
          ...
          // Structured link only for high-school entries; cleared otherwise so a
          // row switched to University can't keep a stale school_id.
          school_id: e.level === "high_school" && e.schoolSlug
            ? (schoolIdBySlug.get(e.schoolSlug) ?? null)
            : null,
    ...
    // The editor's `subjects` array is already in the tutor's chosen order
    // (drag-and-drop in the SubjectPicker); persist that as `position` so the
    // card + profile render it the same way. See migration 0014.
    for (const slug of tutor.subjects ?? []) {
      const sid = bySlug.get(slug);
      if (sid) subjectRowsToInsert.push({ tutor_id: id, subject_id: sid, position: subjectRowsToInsert.length });
...
function tutorRowToCard(row) {
  ...
  return {
    ...
    highSchool,
    university,
    credentials: bridgeAtarIntoCredentials(row.credentials, row.atar).filter((c) => c?.label),
...
function rowToCompletion(row, name) {
  return {
    ...
    credentials: bridgeAtarIntoCredentials(row.credentials, row.atar),
    bioLong: row.bio_long ?? "",
    subjects: row.subjects ?? [],
...
export async function getTutorsForBrowse(supabase, params = {}) {
  ...
  const ranked = rankTutors(data.map(tutorRowToCard));
