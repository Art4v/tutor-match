...
import { expandSubjectSlugs } from "@/lib/subjectFamilies";
...
// Drop blank rows and normalise an ATAR label to 2 decimals for consistent
// display. Exported alongside extractAtarFromCredentials because the COMPANY
// tutor save path (lib/supabase/companies.js) writes the same column through a
// different route, and a second copy of either rule would let "99.5" and
// "99.50" both exist depending on who typed it.
export function normaliseCredentials(credentials) {
  return (credentials ?? [])
    .filter((c) => c?.label)
    .map((c) =>
...
export function extractAtarFromCredentials(credentials) {
...
function buildSaveProfilePayload(tutor) {
  ...
  // the scalar `atar` mirror is derived from the same array for the /browse
  // filter (0036).
...
/**
 * The seeded school catalog — feeds the /browse School filter and the
...
function tutorRowToCard(row) {
  ...
  return {
    ...
    // A company's tutor (0065). The card renders this chip in place of the
    // verified rosette, and `verified` below is forced true so the default
    // verified-only browse filter doesn't hide them: only invited companies exist,
    // so every company tutor on the site belongs to one we chose to list.
    company: row.company ? { slug: row.company.slug, name: row.company.name } : null,
    verified: row.partner_id != null || row.verification_status === "verified",
    bio: row.bio ?? "",
...
function rowToCompletion(row, name) {
  return {
    ...
    // Company tutors ride along for the same reason they pass the verified
    // filter: an invited company is the endorsement. Within the verified block
    // they still sort on avatarBoost and the seeded tie-break, so they
    // interleave with verified tutors rather than displacing them.
    verified: row.partner_id != null || row.verification_status === "verified",
    name,
    bio: row.bio ?? "",
    suburb: row.suburb ?? "",
 ...
 *                            A GENERAL umbrella slug ('general-english') also
 *                            matches its whole cross-exam family — see
 *                            `lib/subjectFamilies.js`
 *   states?:       string[]  AU state codes ('NSW', …) — keep tutors BASED in
 *                            ANY of them (matched against `city`, which stores
 *                            the state code; online tutors are NOT OR-ed in)
...
export async function getTutorsForBrowse(supabase, params = {}) {
  ...
  // 1. Subject filter — tutors with ANY of the requested subjects. The slugs
  //    are widened first so a GENERAL umbrella pick ("English") matches every
  //    exam's English course; exam-specific slugs pass through unchanged. The
  //    widening is query-side only — the URL and the filter chip keep the one
  //    slug the user actually picked.
  if (subjectSlugs.length > 0) {
    const { data: subjectRows, error: sErr } = await supabase
      .from("subjects")
      .select("id")
      .in("slug", expandSubjectSlugs(subjectSlugs));
  ...
  // State = where the tutor is BASED, so it's a plain equality filter (an
  // online-only VIC tutor does not show under NSW). No id-resolution step is
  // needed: `city` is a column on tutor_profiles, holding the state code
  // written by the suburb picker (components/profile-edit/sections.js). Older
  // rows can hold the unabbreviated name instead, so match both forms — still
  // an exact `in`, so the 0004 (city) index stays usable. A tutor who never
  // picked a base suburb has a blank `city` and matches no state.
  if (Array.isArray(states) && states.length > 0) {
    query = query.in("city", states.flatMap((c) => [c, stateName(c)]));
  }
  ...
  // A company's tutors count as verified (0065): companies are invite-only, so
  // every one of them belongs to a company we chose to list. This has to be in
  // the SQL, not just tutorRowToCard — the filter runs first, and without the
  // OR the default verified-only browse would hide every company tutor and
  // quietly defeat the feature.
  if (verifiedOnly) {
    query = query.or("verification_status.eq.verified,partner_id.not.is.null");
  }
  ...
  const { data, error } = await supabase
    ...
    .select(
      `
        *,
        profile:profiles!inner ( full_name ),
        company:partners ( id, slug, name, website_url, logo_url ),
        subjects:tutor_subjects (
          position,
          subject:subjects ( id, name, slug, exam_code )
        ),
...
function tutorRowToDetail(row) {
  ...
  return {
    ...
    // The company this tutor belongs to, or null for an ordinary independent
    // tutor (0065). Non-null changes three things on the profile page: the
    // company chip replaces the verified tick, the company's rate card replaces
    // the tutor's own, and Enquire replaces Message. An embedded LEFT join, so
    // an independent tutor is entirely unaffected.
    company: row.company
      ? {
          id: row.company.id,
          slug: row.company.slug,
          name: row.company.name,
          websiteUrl: row.company.website_url ?? null,
          logoImg: row.company.logo_url ?? null,
        }
      : null,
