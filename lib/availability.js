// ============================================================================
// Canonical availability-grid labels.
// ----------------------------------------------------------------------------
// The tutor settings editor (app/settings/sections.js) and the public profile's
// AvailabilityGrid (via lib/supabase/tutors.js → normalizeAvailability) both
// label the same grid. They used to hardcode *different* hour arrays, so a slot
// a tutor marked "9 am" was shown to students as "8am". These constants are the
// single source of truth — import them in both places.
//
// The grid covers the full 24 hours of the day in 1-hour slots: 24 rows
// (12 am, 1 am, … 11 pm) × 7 days. One cell = one full hour.
//
// Plain constants, no client-only deps, so this module is safe to import from
// both "use client" components and server-side code.
// ============================================================================

export const AVAILABILITY_DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

// 1-hour slots across the full day → 24 rows.
export const SLOT_MINUTES = 60;
export const AVAILABILITY_SLOTS = (24 * 60) / SLOT_MINUTES; // 24
export const AVAILABILITY_DAY_COUNT = AVAILABILITY_DAYS.length; // 7

function buildHourLabels() {
  const out = [];
  for (let h24 = 0; h24 < 24; h24++) {
    const ampm = h24 < 12 ? "am" : "pm";
    const h12 = h24 % 12 === 0 ? 12 : h24 % 12;
    out.push(`${h12} ${ampm}`);
  }
  return out;
}

export const AVAILABILITY_HOURS = buildHourLabels();

/** A fresh, all-unavailable grid sized to the canonical dimensions. */
export function buildEmptyGrid() {
  return Array.from({ length: AVAILABILITY_SLOTS }, () =>
    Array(AVAILABILITY_DAY_COUNT).fill(0)
  );
}

/**
 * Coerce any stored grid to the canonical 24×7 shape: pads missing rows/cols
 * with 0 (unavailable) and truncates extras, clamping cell values to 0–2. Lets
 * the editor render legacy/short grids without crashing. Returns a fresh array.
 */
export function normalizeGrid(raw) {
  const base = buildEmptyGrid();
  if (!Array.isArray(raw)) return base;
  for (let r = 0; r < AVAILABILITY_SLOTS; r++) {
    const row = raw[r];
    if (!Array.isArray(row)) continue;
    for (let c = 0; c < AVAILABILITY_DAY_COUNT; c++) {
      const v = row[c];
      base[r][c] = v === 1 || v === 2 ? v : 0;
    }
  }
  return base;
}

// ----------------------------------------------------------------------------
// Block <-> grid conversion.
// ----------------------------------------------------------------------------
// The settings editor works in terms of human-friendly "blocks" — per-day lists
// of { start, end } hour ranges (end exclusive, e.g. { start: 17, end: 20 } is
// 5 pm–8 pm). Storage and the public profile still use the 24×7 grid, so the
// editor compiles blocks → grid on every edit. Because one grid row is exactly
// one hour, the conversion is lossless in both directions.

/** Display label for an hour boundary 0–24 ("12 am" … "11 pm", and 24 → "12 am"). */
export function hourLabel(h) {
  const h24 = ((h % 24) + 24) % 24;
  const ampm = h24 < 12 ? "am" : "pm";
  const h12 = h24 % 12 === 0 ? 12 : h24 % 12;
  return `${h12} ${ampm}`;
}

/**
 * Grid → blocks. For each day column, coalesces consecutive available rows
 * (1 or legacy 2 — we treat availability as binary) into { start, end } runs.
 * Returns an object keyed by AVAILABILITY_DAYS, each value a sorted block list.
 */
export function gridToBlocks(raw) {
  const grid = normalizeGrid(raw);
  const out = {};
  for (let c = 0; c < AVAILABILITY_DAY_COUNT; c++) {
    const blocks = [];
    let runStart = -1;
    for (let r = 0; r < AVAILABILITY_SLOTS; r++) {
      const on = (grid[r]?.[c] ?? 0) !== 0;
      if (on && runStart === -1) runStart = r;
      if (!on && runStart !== -1) {
        blocks.push({ start: runStart, end: r });
        runStart = -1;
      }
    }
    if (runStart !== -1) blocks.push({ start: runStart, end: AVAILABILITY_SLOTS });
    out[AVAILABILITY_DAYS[c]] = blocks;
  }
  return out;
}

/**
 * Blocks → grid. Starts from an empty grid and sets rows [start, end) to 1 for
 * each day's blocks. OR-ing means overlapping blocks are harmless. Only ever
 * writes 0/1 (availability is binary — no manual "booked").
 */
export function blocksToGrid(blocksByDay) {
  const grid = buildEmptyGrid();
  if (!blocksByDay) return grid;
  for (let c = 0; c < AVAILABILITY_DAY_COUNT; c++) {
    const blocks = blocksByDay[AVAILABILITY_DAYS[c]] || [];
    for (const { start, end } of blocks) {
      const s = Math.max(0, Math.min(AVAILABILITY_SLOTS, start | 0));
      const e = Math.max(0, Math.min(AVAILABILITY_SLOTS, end | 0));
      for (let r = s; r < e; r++) grid[r][c] = 1;
    }
  }
  return grid;
}
